...
| H3 | Safe areas — home indicator | `cockpit_empty`, all tab bar screens | Tab bar bottom edge likely sits at 874 pt frame edge with no explicit 34 pt home indicator clearance in wireframe. | Add 34 pt bottom inset below tab bar items in Figma frame; in code: `paddingBottom: insets.bottom` via `useSafeAreaInsets()` | **HIGH** |
| M1 | iOS 26 tab bar | `cockpit_empty`, all tab bar screens | Tab bar is a solid opaque bottom bar. iOS 26: tab bar becomes a compact floating glass pill that auto-minimizes on scroll, revealing more content. | At implementation, use Expo Router tab bar with `tabBarStyle: { position: 'absolute' }` + glass treatment. Annotate in wireframe as "iOS 26 floating tab bar". | **MEDIUM** |
