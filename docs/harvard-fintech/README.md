...
## Как это оценивают (и что из этого следует)

Задания проверяют живые люди — тьюторы и subject-matter experts со стороны GetSmarter (не гарвардский факультет), по рубрике, с письменной обратной связью. Ассессмент непрерывный. Проходной балл и веса модулей публично не опубликованы — только в handbook.

Три следствия для работы с этим репозиторием:

1. **Оценивают применение концепций модуля, а не качество продукта.** Сильная продуктовая работа без терминологии и кейсов курса получает средний балл. В академической версии каждое утверждение должно опираться на материал модуля, а Decode — быть иллюстрацией, а не предметом.
2. **Ссылки тащить с собой.** В `research/` много verbatim-цитат (PERG 17.7, Klarna Pay in 3 Terms v1.8.0, FCA Financial Lives 2024, RevenueCat) — все они там с атрибуцией к первоисточнику. Копировать абзац в задание без ссылки нельзя: [T&C GetSmarter](https://www.getsmarter.com/terms-and-conditions-for-students) описывают за плагиат ноль за работу, кап по проходному баллу и отчисление без возврата при повторе.
3. **Политику по AI-сгенерированному контенту проверить в handbook в первый день.** Весь корпус — AI-ассистированный ресёрч; у разных провайдеров правила от «декларируй использование» до академической недобросовестности. Не проверено, самый материальный из трёх рисков.
...
| Тезис капстоуна (проблема → рекомендация → ограничения, RU + EN) | ✅ [capstone-deck-m6.md §Тезис](capstone-deck-m6.md) |
| Сверка с handbook (объёмы, рубрики, дедлайны, **политика по AI**) | ○ первая неделя курса |
