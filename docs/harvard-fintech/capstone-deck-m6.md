...
## Тезис (от него собирается колода)

**Проблема.** В merchant-funded потребительском кредите потребитель — не клиент: продукт оплачивает мерчант за конверсию, поэтому ни один участник цепочки не имеет коммерческого стимула сделать условия понятными в момент решения. Обязательное раскрытие закрывает форму, но не понимание. Механизм воспроизводится в каждом embedded-finance продукте (BNPL, store cards, страховые продления, телеком), а регулирование в краткосроке усугубляет его: правила с 15.07.2026 не ретроспективны → два режима условий одновременно на руках у людей.

**Почему существующие ответы не закрывают.** Регулятор предписывает раскрытие, а не понимание, и делает это в момент максимальной спешки. Инкумбенты (Revolut AIR, Starling/NatWest, Cleo) bank-locked и post-hoc — видят транзакцию после списания, не условие до подписи; структурно не могут играть в pre-signature. Универсальные AI-инструменты (ChatGPT, Adobe AI Assistant) не дают детерминированной математики, персистентности, юрисдикционной базы и проверяемых цитат.

**Рекомендация.** Comprehension layer в точке решения — независимый и от банка, и от лендера слой, который по самому документу до подписи выдаёт детерминированную полную стоимость, флаги условий с ценой ошибки в £ и бинарный ответ про кредитный файл, а затем превращает документ в отслеживаемое обязательство.

**Три ограничения = конструкция решения:** (1) только до подписи → вне периметра debt counselling (PERG 17.7 ex.(4)) → масштабируется без FCA-авторизации; (2) без подключения банка → снят барьер входа у самого уязвимого сегмента (~80% взрослых UK не пользуются open banking); (3) без ссылок на продукты, партнёрок и «apply» → нет credit broking и s21, сохранена нейтральность, которая и есть ценность слоя.

**Кто выигрывает.** Потребитель; регулятор (Consumer Duty требует доказать consumer-understanding outcome — такой слой измеряет понимание со стороны потребителя, а не со стороны раскрытия); добросовестные лендеры (их нормальные условия становятся видимыми — поэтому детектор показывает и «зелёные» пункты). Проигрывают те, чья маржа лежит в непонимании.

**Рамка подачи.** Не «приложение, которое сканирует документы», а потребительский слой инфраструктуры на рынке, который регулятор только что переформатировал. Соскальзывание в «удобный сканер» обесценивает и слайд 5, и слайд 10.

### Формулировка для сдачи (EN)

> Mandatory disclosure regulates the *form* of information, not its *comprehension*, at the exact moment consumers are least able to process it. In merchant-funded credit, no party in the chain is paid to close that gap. The recommendation is an independent comprehension layer at the point of decision: deterministic cost arithmetic computed in code, line-level citations back to the consumer's own document, and a binary answer on irreversible consequences — delivered pre-signature, without bank access, and with no product referrals. The constraints are the solution: pre-signature keeps it outside the regulated advice perimeter, no bank access removes the adoption barrier, and no referrals preserve the neutrality that gives the layer its value.

### То же на уровне человека (для слайда 7 и для Q&A)

21 год, три плана BNPL в Klarna, Clearpay и PayPal, дат не помнит, боится, что уже испортил кредитную историю. Палец над кнопкой «Pay in 3». Четыре вопроса, на которые сегодня не отвечает никто: **сколько это стоит на самом деле** (£412, а не £300 с витрины) · **попадёт ли в кредитный файл** (YES/NO) · **что будет, если пропущу один платёж** (0% → 29% годовых, проценты с даты покупки) · **когда спишут** (дата, сумма, напоминание). «Я не понял» и «я забыл» — две разные проблемы, и обе наши.
