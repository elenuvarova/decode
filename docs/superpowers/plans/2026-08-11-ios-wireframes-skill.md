...
- **Каждое число** в `reference/canvas-and-metrics.md` помечено происхождением: `[кит]` · `[замер]` · `[выведено]` · `[номинал]`. Без пометки числа не добавлять.
...
- Produces: таблицы чисел, на которые ссылаются фазы 2–3 в `SKILL.md`. Формат каждой строки: `| что | значение | происхождение |`, где происхождение — ровно один из тегов `[кит]` / `[замер]` / `[выведено]` / `[номинал]`.
...
Шапка файла: версия кита и дата извлечения (Apple Design Resources iOS 26, файл `pL2Suk43Ua44Cwld1vTAtb`), легенда тегов, и предупреждение — при новой major-версии iOS перепроверять `[кит]`-строки.

**Таблица «Канвас и safe area»:**

| что | значение | тег |
|---|---|---|
| Кадр экрана iPhone | 402 × 874 (16/17 Pro) | `[кит]` |
| Кадров шириной 402 в ките — ровно по 874 | 43 из 43 | `[кит]` |
| Боковой инсет экрана → ширина контента | 16 / 16 → 370 | `[кит]` |
| Слой home-индикатора | 402 × 34, y = 840 | `[кит]` |
| Полоса индикатора | 144 × 5, 8 от нижней кромки | `[кит]` |
| Dynamic Island сверху | ≈ 59 | `[номинал]` |
| Минимальный тач-таргет | 44 × 44 | `[номинал]` |

**Таблица «Лист (modal sheet)»:**

| вариант | верх → низ | примечание | тег |
|---|---|---|---|
| Fullscreen | 62 → 874 | до нижней кромки | `[кит]` |
| Fullscreen Stack | 62 → 874 | до нижней кромки | `[кит]` |
| Inspector (врезной, iOS 26) | 500 → 874, подложка врезана 6 слева/справа, кончается на 868 | меньшинство: 3 приложения из 20 | `[кит]` + `[замер]` |
| Home-индикатор | брат листа **поверх**, не ребёнок под ним | во всех трёх вариантах | `[кит]` |

Отдельной строкой записать замер живых приложений: у 4 промеренных по пикселям лист белый до самой кромки, ни у одного он не обрывается полосой фона `[замер]`.

**Таблица «Строка списка»:**

| что | значение | тег |
|---|---|---|
| Боковой инсет строки | 0/16/0/16 | `[кит]` |
| Ведущий бокс символа | 28 × 20 | `[кит]` |
| Ведущий глиф | SF Pro 17 Regular, центр/центр | `[кит]` |
| Зазор символ → текст | 8 | `[кит]` |
| Начало текстовой колонки от края карточки | 16 + 28 + 8 = 52 | `[кит]` |
| Зазор между трейлинг-элементами | 16 | `[кит]` |
| Разделитель | внутри контентной колонки: от 52 и не доходит 16 до правого края | `[кит]` |
| Шеврон drill-in | SF Pro 17 Semibold, ширина адванса 8; фактический ink при 17 pt = 8,69 × 14,91 | `[кит]` |
| Инсет до иконки у 7 промеренных приложений | 16, без исключений (1 px ≈ 1,31 pt, ±1,5 pt) | `[замер]` |
| Зазор иконка → текст в живых | 12–16 | `[замер]` |
| Размер символа: голый / в плашке | 18–24 / 28–32 — **вкусовщина**, источники разошлись | `[замер]` |
...
| что | значение | тег |
|---|---|---|
| Высота крупной CTA | 48 | `[кит]` |
| Secondary — заливка | `#787880` @ 16 % (= `#E9E9EB` на белом) | `[кит]` |
| Destructive — заливка | та же нейтральная, подпись красная | `[кит]` |
| Обводочная CTA | **отсутствует в ките во всех наборах** | `[кит]` |
| Нейтральная заливка в живых приложениях | (228,228,228) · (233,239,244) · (242,248,242) — кучно вокруг `#E9E9EB` | `[замер]` |
| Обводка как третий уровень | наше расширение ради читаемости в сером; на hi-fi схлопывается в текстовую с тинтом | `[выведено]` |

**Таблица «Прочие компоненты»:**

| что | значение | тег |
|---|---|---|
| Поле формы (Text Field) | строка 52, инсет 16, текст 17 | `[кит]` |
| Поле поиска | 370 × 44, капсула | `[кит]` |
| Алерт | окно 300, радиус 34, паддинг 14, текст слева 17/22, кнопки в ряд капсулами 48 | `[кит]` |
| Радиусы листа | верх 34, низ 58 | `[кит]` |
| Радиусы клавиатуры | верх 27, низ 62 | `[кит]` |
| Панель ввода (композер) | высота капсулы 44 — взята у поля поиска, **в ките такого компонента нет** | `[выведено]` |

**Таблица «Типографика (SF Pro — размер / интерлиньяж / трекинг)»:** Large Title Emphasized Bold 34/41/+0.40 · Title 3 Regular 20/25/−0.45 · Headline Semibold 17/22/−0.43 · Body Regular 17/22/−0.43 · Subheadline Regular 15/20/−0.23 · Caption 2 Regular 11/13/+0.06 — все `[кит]`.

**Таблица «Семантические цвета, Light»:** BG Primary `#ffffff` · BG Grouped Primary `#f2f2f7` · Label Primary `#000000` · Label Secondary `#3c3c4399` · Label Tertiary `#3c3c434d` · Separator Opaque `#c6c6c8` · Separator Non-opaque `#0000001f` — все `[кит]`.
...
Expected: у всех четырёх счётчик больше нуля. Если `[выведено]` или `[замер]` = 0 — файл потерял различение между китом и нашими выводами, вернуться к шагу 3.
...
node --check "$F" && echo "синтаксис ок"
grep -niE 'decode|elenauvarova|FCA|StepChange|MoneyHelper|/Users/|git projects' "$F" && echo "НАЙДЕНА СПЕЦИФИКА" || echo "специфики нет"
```

- [ ] **Step 2: Запустить и убедиться, что падает**

Run: команду из шага 1.
Expected: FAIL — `node --check` сообщает, что файла нет.
...
Workflow требует, чтобы `meta` не содержал переменных, вызовов и интерполяции.

```bash
awk '/^export const meta/,/^}/' ~/.claude/skills/ios-wireframes/assets/audit-workflow.mjs | grep -nE '\$\{|\+|\(' && echo "META НЕ ЛИТЕРАЛ" || echo "meta литерал ок"
```

Expected: `meta литерал ок`.
...
awk '/^\|/ && /[0-9]/ && !/^\|[- :|]*\|$/ && !/\[кит\]|\[замер\]|\[выведено\]|\[номинал\]/ { n++ } END { print (n>0 ? "НЕПОМЕЧЕННЫХ: " n : "все числа помечены") }' \
  "$SK/ios-wireframes/reference/canvas-and-metrics.md"
echo "--- 4. воркфлоу ---"; node --check "$SK/ios-wireframes/assets/audit-workflow.mjs" && echo "парсится"
