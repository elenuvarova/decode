...
**Персоны:** все; Jordan (досье обязательства из cockpit-строки); J6 «глазами кредитора» — P2-глубина · **Доступ:** автор документа; **Face ID-гейт на входе в Vault** (`expo-local-authentication`) — дешёвый trust-сигнал
...
### Данные
- `commitment`: `{ id, doc_id, auto_name, provider, type, status, next_date, next_amount, trap_flags[], cadence }` — та же сущность, что строка cockpit.
...
- **empty vault:** нет документов → guided empty (= empty cockpit): «Scan your first document…» + `[Scan]` — анти-тупик, detail недостижим.
- **single:** один документ — detail полный, Compare offers обоснованно скрыт/disabled («nothing to compare yet»).
- **many:** дефолт-путь из Vault list / cockpit-строки.
...
### Связи
VA-d ← [cockpit](cockpit.md) строка `CommitmentRow` · Vault list (VA) · [alert detail](alerts.md) «Review terms». VA-d → [decode-result](decode-result.md) (re-open) · [Ask](ask.md) · Source-Highlight · `delete-document-confirm` · Document-History. После delete → возврат в Vault list.
...
- **Bottom-sheet** поверх Vault/cockpit; grabber; в шапке — авто-имя «Klarna BNPL offer — 14 May 2026» + provider/type.
...
- ~~TODO: Vault — таб vs секция Home~~ ✅ резолвлено 2026-07-02 (sitemap §4.2): Vault остаётся отдельным табом — cockpit = обзор списаний/статусов, vault = архив документов.
