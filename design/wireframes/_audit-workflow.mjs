...
const VGROUPS = [
  { label: 'visual:core', files: ['cockpit-wireframe.png', 'decode-result-wireframe.png', 'ask.png', 'scan-capture.png', 'scan-review.png'] },
  ...
  { label: 'visual:watch-vault', files: ['alerts-inbox.png', 'alert-detail.png', 'vault-list.png', 'vault-detail.png', 'cockpit-empty.png'] },
...
const crossPrompt = 'Audit CROSS-SCREEN CONSISTENCY and FLOW COVERAGE. Read ' + DOCS + '/docs/ux/screen-sitemap.md and ' + DOCS + '/docs/ux/jtbd-to-flows.md. Then Read PNGs: ' + png('cockpit-wireframe.png') + ', ' + png('vault-list.png') + ', ' + png('cockpit-empty.png') + ', ' + png('alerts-inbox.png') + ', ' + png('settings.png') + ', ' + png('scan-capture.png') + ', ' + png('onboarding-1-welcome.png') + ', ' + png('onboarding-4-register.png') + ', ' + png('decode-result-wireframe.png') + ', ' + png('ask.png') + '. Check: (1) TabBar (Home / + Scan / Vault) consistent and present on tab-level screens (Cockpit, Cockpit-Empty, Vault-List); is the ACTIVE tab indicated? (2) header patterns consistent (back chevron vs X for modals). (3) terminology consistent. (4) every P0 JTBD flow (J1 scan-decode, J2 cockpit, J3 radar, J4 ask) has its screens. source = doc or PNG path.'
