...
- **Event-таксономия** в `object_action` past-tense snake_case: воронка `app_opened → signed_up → capture_started → decode_completed → true_cost_revealed (ACTIVATION) → commitment_saved → reminder_set → paywall_viewed → subscription_started` + Watch-слой (`reminder_opened` = главный return-триггер, `overview_viewed`, `price_rise_detected`). Биллинг-события серверные через RevenueCat webhooks (иначе теряются отмены вне сессии). **Минимум для Day-0 — 11 событий.**
...
- **Retention — НЕ N-day, а unbounded помесячно + ценностное return-событие** (`decode_completed`/`reminder_opened`/`overview_viewed`, не голый `app_opened`), т.к. Decode — low-frequency utility (Airbnb-кейс Amplitude). Day-7 7%-правило — только ранний PMF-предохранитель на 15.07. Главная когорта: **loop-closed vs scan-only** (её расхождение доказывает, что Watch-слой = моат).
